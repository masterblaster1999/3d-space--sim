...
int main(int argc, char** argv) {
  ...
  int routeDestStationIndex = 0; // in-system route planner destination
...
policeAlertUntilDays = 0.0;
        ...
        // Time acceleration: [ / ] (ignored while typing into UI).
        if (!io.WantCaptureKeyboard) {
          auto stepTimeScale = [&](int dir) {
            constexpr double kSteps[] = { 1.0, 2.0, 5.0, 10.0, 20.0, 40.0, 60.0, 120.0, 240.0, 480.0, 600.0 };
            int best = 0;
            double bestDiff = std::abs(timeScale - kSteps[0]);
            for (int i = 1; i < (int)(sizeof(kSteps) / sizeof(kSteps[0])); ++i) {
              const double diff = std::abs(timeScale - kSteps[i]);
              if (diff < bestDiff) { bestDiff = diff; best = i; }
            }
            best = std::clamp(best + dir, 0, (int)(sizeof(kSteps) / sizeof(kSteps[0])) - 1);
            timeScale = kSteps[best];
          };

          if (event.key.keysym.sym == SDLK_LEFTBRACKET) {
            stepTimeScale(-1);
            toast(toasts, "Time accel: " + std::to_string((int)timeScale) + "x", 1.2);
          }
...
if (showShip) {
  ...
  ImGui::BulletText("H supercruise-lite (requires target)");
  ImGui::BulletText("[ / ] step time accel");
  ...
  if (ImGui::SliderFloat("Time accel (sim sec / real sec)", &ts, 1.0f, 600.0f, "%.0f")) {
    timeScale = (double)ts;
  }
  ImGui::TextDisabled("Tip: [ / ] steps time accel.");
...
if (canTrade) {
  ...
  if (bounty > 0.0) {
    ...
    ImGui::TextDisabled("No bounty.");
        ...
        // Trade route planner (in-system): find profitable goods to haul to another station.
        ImGui::Separator();
        if (ImGui::CollapsingHeader("Route Planner", ImGuiTreeNodeFlags_DefaultOpen)) {
          if (currentSystem->stations.size() < 2) {
            ImGui::TextDisabled("No other stations in this system.");
          } else {
            const int stationCount = (int)currentSystem->stations.size();

            routeDestStationIndex = std::clamp(routeDestStationIndex, 0, stationCount - 1);
            if (routeDestStationIndex == selectedStationIndex) {
              routeDestStationIndex = (routeDestStationIndex + 1) % stationCount;
            }

            const char* destLabel = currentSystem->stations[(std::size_t)routeDestStationIndex].name.c_str();
            if (ImGui::BeginCombo("Destination", destLabel)) {
              for (int si = 0; si < stationCount; ++si) {
                if (si == selectedStationIndex) continue;

                bool selected = (si == routeDestStationIndex);
                const auto& st2 = currentSystem->stations[(std::size_t)si];
                if (ImGui::Selectable(st2.name.c_str(), selected)) {
                  routeDestStationIndex = si;
                }
            ...
            const auto& destStation = currentSystem->stations[(std::size_t)routeDestStationIndex];

            ImGui::SameLine();
            if (ImGui::SmallButton("Set target")) {
              target.kind = TargetKind::Station;
              target.index = (std::size_t)routeDestStationIndex;
              toast(toasts, "Target set: " + destStation.name, 1.6);
            }

            const double feeTo = effectiveFeeRate(destStation);
            auto& econTo = universe.stationEconomy(destStation, timeDays);

            const math::Vec3d aPos = stationPosKm(station, timeDays);
            const math::Vec3d bPos = stationPosKm(destStation, timeDays);
            const double distKm = (bPos - aPos).length();
            ImGui::TextDisabled("Approx distance: %.0f km | Fees: buy %.2f%%, sell %.2f%%",
                                distKm, feeEff * 100.0, feeTo * 100.0);

            struct RouteRow {
              econ::CommodityId cid{};
              double buyCost{0.0};      // per unit (ask + fees)
              double sellPayout{0.0};   // per unit (bid - fees)
              double profitPerUnit{0.0};
              double profitPerKg{0.0};
              double maxUnits{0.0};     // constrained by stock, credits, destination capacity, and cargo kg
              double estProfit{0.0};    // profitPerUnit * maxUnits
            };

            std::vector<RouteRow> rows;
            rows.reserve(econ::kCommodityCount);

            auto cidx = [](econ::CommodityId id) -> std::size_t { return (std::size_t)id; };
            const double freeKg = std::max(0.0, cargoCapacityKg - cargoKgNow);

            for (std::size_t i = 0; i < econ::kCommodityCount; ++i) {
              const auto cid = (econ::CommodityId)i;
              const auto def = econ::commodityDef(cid);
              if (def.massKg <= 0.0) continue;

              const auto qFrom = econ::quote(stEcon, station.economyModel, cid, 0.10);
              const auto qTo   = econ::quote(econTo, destStation.economyModel, cid, 0.10);

              // Apply fees as the actual cost/payout per unit.
              const double buyCost = qFrom.ask * (1.0 + feeEff);
              const double sellPayout = qTo.bid * (1.0 - feeTo);
              const double profit = sellPayout - buyCost;
              if (profit <= 0.0) continue;

              // Limits: cargo mass, credits, station stock, and destination free capacity.
              const double unitsCargo = std::floor(freeKg / def.massKg);
              const double unitsCredits = (buyCost > 1e-9) ? std::floor(credits / buyCost) : 0.0;

              const double destCap = destStation.economyModel.capacity[cidx(cid)];
              const double destCur = econTo.inventory[cidx(cid)];
              const double destFree = std::max(0.0, destCap - destCur);

              double maxUnits = std::min({ qFrom.inventory, destFree, unitsCargo, unitsCredits });
              if (!std::isfinite(maxUnits)) maxUnits = 0.0;
              maxUnits = std::max(0.0, std::floor(maxUnits));

              RouteRow r{};
              r.cid = cid;
              r.buyCost = buyCost;
              r.sellPayout = sellPayout;
              r.profitPerUnit = profit;
              r.profitPerKg = profit / def.massKg;
              r.maxUnits = maxUnits;
              r.estProfit = profit * maxUnits;
              rows.push_back(r);
            }

            std::sort(rows.begin(), rows.end(), [](const RouteRow& a, const RouteRow& b) {
              return a.estProfit > b.estProfit;
            });

            constexpr int kMaxShow = 8;
            if ((int)rows.size() > kMaxShow) rows.resize(kMaxShow);

            if (rows.empty()) {
              ImGui::TextDisabled("No profitable in-system routes found (after fees).");
            } else if (ImGui::BeginTable("routes", 8, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
              ImGui::TableSetupColumn("Commodity");
              ImGui::TableSetupColumn("Buy");
              ImGui::TableSetupColumn("Sell");
              ImGui::TableSetupColumn("Net/u");
              ImGui::TableSetupColumn("Net/kg");
              ImGui::TableSetupColumn("Max u");
              ...
              for (std::size_t ri = 0; ri < rows.size(); ++ri) {
                ...
                const auto def = econ::commodityDef(r.cid);
                ...
                ImGui::Text("%.2f", r.buyCost);

                ImGui::TableSetColumnIndex(2);
                ImGui::Text("%.2f", r.sellPayout);

                ImGui::TableSetColumnIndex(3);
                ImGui::Text("%.2f", r.profitPerUnit);

                ImGui::TableSetColumnIndex(4);
                ImGui::Text("%.2f", r.profitPerKg);

                ImGui::TableSetColumnIndex(5);
                ImGui::Text("%.0f", r.maxUnits);

                ImGui::TableSetColumnIndex(6);
                ImGui::Text("%.0f", r.estProfit);

                ImGui::TableSetColumnIndex(7);
                ImGui::BeginDisabled(!canTrade || r.maxUnits <= 0.0);

                ImGui::PushID((int)ri);
                if (ImGui::SmallButton("Buy max")) {
                  const double buyUnits = r.maxUnits;
                  const double addKg = buyUnits * def.massKg;
                  if (cargoKgNow + addKg > cargoCapacityKg + 1e-6) {
                    toast(toasts, "Cargo hold full (mass limit).", 2.0);
                  } else {
                    auto tr = econ::buy(stEcon, station.economyModel, r.cid, buyUnits, credits, 0.10, feeEff);
                    if (tr.ok) {
                      cargo[cidx(r.cid)] += buyUnits;
                      cargoKgNow += addKg;
                      toast(toasts,
                            "Bought " + std::to_string((int)buyUnits) + " " + std::string(def.name) + ".",
                            1.8);
                    }
                  }
                }
          ...
          for (auto& m : missions) {
            ...
            if (active) {
              if (ImGui::SmallButton("Set destination")) {
                ...
                sim::StationId destStation = m.toStation;

                // Multi-hop: while on leg 0, the "destination" is the via stop.
                if (m.type == sim::MissionType::MultiDelivery && m.viaSystem != 0 && m.viaStation != 0 && m.leg == 0) {
                  destSys = m.viaSystem;
                  destStation = m.viaStation;
                }

                galaxySelectedSystemId = destSys;

                bool setLocal = false;
                if (destSys == currentSystem->stub.id && destStation != 0) {
                  for (std::size_t si = 0; si < currentSystem->stations.size(); ++si) {
                    if (currentSystem->stations[si].id == destStation) {
                      target.kind = TargetKind::Station;
                      target.index = si;
                      routeDestStationIndex = (int)si;
                      toast(toasts, "Target set: " + currentSystem->stations[si].name, 2.0);
                      setLocal = true;
                      break;
                    }
                  }
